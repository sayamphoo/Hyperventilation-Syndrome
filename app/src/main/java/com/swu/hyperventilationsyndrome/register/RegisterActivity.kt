...
class RegisterActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        binding.confirm.setOnClickListener {
            if (binding.name.text.trim().isNotEmpty() && binding.age.text.trim()
                    .isNotEmpty() && binding.spinner.selectedItem.toString() != "Select Sec"
            ) {
    ...
    private fun getApi() {
        ...
        call.enqueue(object : Callback<List<DataModel>> {
            override fun onResponse(
                call: Call<List<DataModel>>, response: Response<List<DataModel>>
            ) {
                if (response.isSuccessful) {
                    ...
                    when (sharedPreferences.getInt("age", 0)) {
                        ...
                        in 6..12 -> {
                            ...
                            if (frequency in 13.0..20.0) isFrequency = true
                            Log.d("sayamphooooo","sasa" + frequency.toString())
                        }
                    }
                    Log.d("sayamphooooo", sharedPreferences.getInt("age", 0).toString())
                    Log.d("sayamphooooo",math.amplitude().toString())


                    if ((isFrequency && math.amplitude() < 5.0)) {
                        dialog.dialogInformation(
                            R.drawable.accept, "Normal", ProfileActivity::class.java
                        )
                    } else {
                        dialog.dialogInformation(
                            R.drawable.warning, "Warning", TrainingActivity::class.java
                        )
                    }
